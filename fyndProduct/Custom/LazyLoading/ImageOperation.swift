...
class ImageOperation: Operation {
    ...
    required init (url: URL, indexPath: IndexPath?, collectionTag: Int) {
        self.imageUrl = url
    ...
    func downloadImageFromUrl() {
        ...
        let downloadTask = newSession.downloadTask(with: self.imageUrl) { (location, response, error) in
            if let locationUrl = location, let data = try? Data(contentsOf: locationUrl){
                ...
                self.downloadHandler?(image,self.imageUrl, self.indexPath, self.collectionTag, error)
            }
            self.finish(true)
